...
| 韩国 | KOFIA FreeSIS、KSD SEIBro、Naver Finance KRX 指数交叉校验 | GitHub Actions 自动刷新 | 已验证 / 二级行情参照 |
...
- KOSPI 与 KOSDAQ 只作为杠杆读数的市场背景：KOFIA FreeSIS 日别市场统计提供约一年日频序列，最新值同时与 Naver Finance 指数接口交叉校验。它们不是 KRX 原始结算文件，不标为可交易执行价。
...
4. 从 KOFIA FreeSIS 的 KOSPI/KOSDAQ 日别市场服务读取约一年日频序列，并以 Naver Finance 指数接口交叉校验最新值；各响应单独归档，不能将它们等同于 KRX 原始结算文件。
