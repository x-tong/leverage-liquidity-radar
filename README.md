...
| 韩国 | KOFIA FreeSIS、Naver Finance KRX 指数接口 | GitHub Actions 自动刷新 | 已验证 / 供应商行情 |
...
5. 只在 `src/generated/` 或 `data/raw/` 有变化时提交数据快照。失败时不会覆盖上一个有效快照，GitHub Actions 会失败并留下可审计日志。
